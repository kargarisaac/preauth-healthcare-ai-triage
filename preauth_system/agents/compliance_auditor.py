"""
Compliance Auditor Agent Configuration and Class
Dynamically loaded by DSPyAgent base class
"""
import dspy
from datetime import datetime
from preauth_system.signatures import ComplianceAudit, AgentResult
...
class ComplianceAuditor:
    """Compliance Auditor Agent with centralized LM configuration and scoped DSPy usage."""

    def __init__(self):
        self.metadata = METADATA
        self.lm = get_openrouter_lm(agent_name=self.metadata["name"])
        self.react = dspy.ReAct(
            signature=ComplianceAudit,
            tools=DEFAULT_TOOLS,
            max_iters=self.metadata.get("max_iters", 4),
        )
    ...
    async def analyze(self, context, previous_results):
        ...
        try:
            ...
            authorization_decision = previous_results.get("decision-maker", {}).get(
                "response", ""
            )
            ...
            return AgentResult(
                ...
                response=result.compliance_audit,
                usage={
