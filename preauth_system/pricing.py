...
# Model aliases for common variations (OpenAI-native only; OpenRouter handled via provider usage)
MODEL_ALIASES = {
    "gpt-5": "gpt-5",
    "gpt5": "gpt-5",
...
def get_model_pricing(model: str) -> Dict[str, float]:
    """
    Get current pricing for a specific model (OpenAI-native only).
    For non-OpenAI providers (e.g., OpenRouter), caller should rely on provider-reported usage/costs.
    """
    # Normalize model name
    normalized_model = MODEL_ALIASES.get(model, model)

    if normalized_model not in OPENAI_PRICING:
        # Fallback to gpt-5 pricing for unknown OpenAI models
...
def estimate_reasoning_tokens(
    base_output_tokens: int, reasoning_effort: str = "medium"
) -> int:
    """
...
def calculate_cost_from_usage(
    usage: Dict[str, Any], model: Optional[str] = None
) -> Dict[str, Any]:
    """
    Calculate cost from usage object.

    Rules:
    - If provider is OpenRouter and provider cost info is present, return provider-reported credits without hardcoding rates.
    - For OpenAI-native models, compute USD using internal pricing.
    - Otherwise, return zero-cost with notes.
    """
    provider = usage.get("provider")
    # Prefer explicit model, but fall back to usage
    model = model or usage.get("model", "gpt-5")

    # OpenRouter path: do not hardcode rates
    if provider == "openrouter":
        credits = usage.get("provider_cost_credits")
        cost_details = usage.get("provider_cost_details")
        result = {
            "model": model,
            ...
            "token_counts": {
                ...
                "fresh_input_tokens": max(
                    ...
                    - int(usage.get("input_cached_tokens", 0)),
                    0,
                ),
            ...
            "costs_breakdown": {
                # Keep USD at 0.0 to avoid misleading conversion
                "input_cost_usd": 0.0,
                "cached_cost_usd": 0.0,
                "output_cost_usd": 0.0,
                "reasoning_cost_usd": 0.0,
                "total_cost_usd": 0.0,
                # Provider-reported credits (authoritative for OpenRouter)
                "total_cost_credits": credits,
                "provider_cost_details": cost_details,
                "currency": "credits",
            },
    ...
    # OpenAI-native path: compute USD using internal pricing
    ...
    return {
        ...
        "calculated_at": datetime.now().isoformat(),
        "cost_optimization_notes": _generate_cost_optimization_notes(
            model, reasoning_effort, usage
        ),
    }


def _generate_cost_optimization_notes(
    model: str, reasoning_effort: str, usage: Dict[str, Any]
) -> list:
    ...
    if reasoning_effort == "high" and model.startswith("gpt-5"):
        notes.append(
            "Consider using 'medium' or 'low' reasoning_effort for cost savings if task doesn't require deep reasoning"
        )
    ...
    if cached_tokens == 0 and total_input > 1000:
        notes.append(
            "Consider structuring repeated prompts to benefit from 90% caching discount"
        )
...
def get_all_model_pricing() -> Dict[str, Dict[str, float]]:
    """Get pricing for all supported OpenAI-native models."""
