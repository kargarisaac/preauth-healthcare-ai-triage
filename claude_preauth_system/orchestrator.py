...
try:
    ...
    logger.error(
        "Warning: Claude Code SDK not available. Install with: pip install claude-code-sdk"
    )
    SDK_AVAILABLE = False
    raise ImportError("Claude Code SDK not available")
...
class PreAuthOrchestrator:
    ...
    def __init__(self, current_request_path: str, patient_folder_path: str):
        ...
        self.output_dir = Path(
            f"analysis_results/analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    async def validate_inputs(self) -> bool:
        ...
        try:
            ...
            if not self.current_request_path.exists():
                logger.error(
                    f"Current request file not found: {self.current_request_path}"
                )
                return False
            ...
            try:
                ...
                logger.info(
                    f"JSON Bundle validated: {request_data.get('id', 'unknown')}"
                )
    ...
    def prepare_agent_context(self, agent_name: str) -> str:
        ...
        if dependencies:
            context_data["previous_analyses"] = {
                dep: self.agent_results.get(dep, {}) for dep in dependencies
            }
        ...
        for agent_name in self.agents.keys():
            ...
            if agent_file.exists():
                try:
                    ...
                    self.agent_definitions[agent_name] = self._parse_agent_definition(
                        content
                    )
                    logger.info(f"Loaded specialized definition for {agent_name}")
                except Exception as e:
                    logger.error(f"Failed to load {agent_name} definition: {e}")
                    # Fallback to basic definition
                    self.agent_definitions[agent_name] = {
                        "name": agent_name,
                        "description": self.agents[agent_name]["description"],
                        "instructions": f"You are a {agent_name.replace('-', ' ')} specialist.",
                        "tools": ["Read", "Grep", "Glob"],
                    }
            else:
                logger.warning(f"Agent definition file not found: {agent_file}")
                # Fallback to basic definition
                self.agent_definitions[agent_name] = {
                    "name": agent_name,
                    "description": self.agents[agent_name]["description"],
    ...
    def _parse_agent_definition(self, content: str) -> Dict[str, Any]:
                ...
                for line in lines[1 : frontmatter_end - 1]:
                    if ":" in line:
                        ...
                        if key == "name":
                            ...
                            # Parse tools list [Read, Grep, Glob]
        ...
        # Join the remaining content as instructions
        definition["instructions"] = "\n".join(content_lines).strip()

        return definition

    def build_agent_prompt(self, agent_name: str) -> str:
        """Build comprehensive prompt using specialized agent definition"""
        context = self.prepare_agent_context(agent_name)
        agent_def = self.agent_definitions.get(agent_name, {})

        # Get agent instructions from loaded definition
        agent_instructions = agent_def.get("instructions", "")
        agent_description = agent_def.get(
            "description", self.agents[agent_name]["description"]
        )

        # Build the comprehensive prompt combining context with specialized instructions
...
**START ANALYSIS NOW** - Begin by reading the patient data files and conducting your specialized medical analysis according to your role and expertise above."""
    ...
    def clean_agent_response(self, raw_response: str) -> str:
        ...
        if not cleaned_lines:
            ...
            for block in sorted(text_blocks, key=len, reverse=True):
                if any(
                    keyword.lower() in block.lower() for keyword in medical_keywords
                ):
                    return block
        ...
        return (
            "\n".join(cleaned_lines)
            if cleaned_lines
            else "Analysis completed but detailed report not properly formatted."
        )
    ...
    async def run_agent(self, agent_name: str) -> Dict[str, Any]:
        ...
        try:
                ...
                if isinstance(message, ResultMessage):
                    if hasattr(message, "usage") and message.usage:
                        usage_data = {
                            ...
                            "service_tier": message.usage.get(
            ...
            with open(result_file, "w") as f:
                ...
                if usage_data:
                    f.write(
                        f"**Token Usage**: {usage_data.get('total_tokens', 'N/A')} tokens\n"
                    )
                    f.write(f"  - Input tokens: {usage_data.get('input_tokens', 0)}\n")
                    f.write(
                        f"  - Cache creation tokens: {usage_data.get('cache_creation_input_tokens', 0)}\n"
                    )
                    f.write(
                        f"  - Cache read tokens: {usage_data.get('cache_read_input_tokens', 0)}\n"
                    )
                    f.write(
                        f"  - Output tokens: {usage_data.get('output_tokens', 0)}\n"
                    )
                    f.write(
                        f"**Cost**: ${usage_data.get('total_cost_usd', 0):.4f} USD\n"
                    )
                    f.write(
                        f"**API Duration**: {usage_data.get('duration_api_ms', 0)} ms\n"
                    )
            ...
            cost_info = (
                ...
                if usage_data.get("total_cost_usd")
                else ""
            )
            token_info = (
                f" - {usage_data.get('total_tokens', 'N/A')} tokens"
                if usage_data.get("total_tokens")
                else ""
            )
            logger.info(
                f"✅ {agent_name} completed successfully in {processing_time:.2f}s{cost_info}{token_info}"
            )
            return agent_result
    ...
    async def run_phase(self, phase_number: int) -> Dict[str, Any]:
        ...
        logger.info(
            f"🚀 Starting Phase {phase_number} with agents: {', '.join(phase_agents)}"
        )
    ...
    async def orchestrate_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_processing_time = (
                datetime.now()
                - datetime.fromisoformat(self.analysis_metadata["start_time"])
            ).total_seconds()
            ...
            self.analysis_metadata.update(
                {
                    ...
                    "successful_agents": len(
                        [r for r in self.agent_results.values() if r.get("success")]
                    ),
                    "failed_agents": len(
                        [r for r in self.agent_results.values() if not r.get("success")]
                    ),
    ...
    async def generate_final_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not successful_analyses:
                raise ValueError(
                    "No successful agent analyses available for final report"
                )

            # Extract key findings from each agent
            clinical_findings = successful_analyses.get("clinical-analyzer", {}).get(
                "response", "Not available"
            )
            medication_findings = successful_analyses.get(
                "medication-specialist", {}
            ).get("response", "Not available")
            risk_findings = successful_analyses.get("risk-assessor", {}).get(
                "response", "Not available"
            )
            decision_findings = successful_analyses.get("decision-maker", {}).get(
                "response", "Not available"
            )
            compliance_findings = successful_analyses.get("compliance-auditor", {}).get(
                "response", "Not available"
            )
...
*This report was generated by the Nazmito AI Pre-Authorization Analysis System.*
"""
...
async def main():
    ...
    if len(sys.argv) != 3:
        print(
            "Usage: python claude_preauth_orchestrator.py <current_request.json> <patient_folder>"
        )
        print("\nExample:")
        print(
            "  python orchestrator.py ../data/processed_data/11f5688b-6c4a-4c41-baad-71e6a4b82d91/abudhabi_11f5688b-6c4a-4c41-baad-71e6a4b82d91_20200214_req01_shafafiya.json ../data/processed_data/11f5688b-6c4a-4c41-baad-71e6a4b82d91/"
        )
        sys.exit(1)
    ...
    try:
        ...
        if result.get("status") == "completed":
            ...
            print(
                f"✅ Successful Agents: {result['successful_agents']}/{result['total_agents']}"
            )
            ...
            total_tokens = sum(
                usage.get("total_tokens", 0)
                for usage in orchestrator.token_usage.values()
            )
            if total_cost > 0:
