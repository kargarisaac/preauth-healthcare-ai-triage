...
class TestCanonicalSchema:
    ...
    def test_resource_type_is_bundle(self):
        ...
        if "enum" in resource_type_prop:
            ...
            assert resource_type_prop["const"] in ["Claim", "Bundle"], "Schema should support Bundle or Claim"
    ...
    def test_patient_reference_format(self):
        ...
        assert patient_ref.startswith("Patient/"), (
    ...
    def test_xml_to_canonical_transformation(self):
        ...
        for format_name, xml_path in self.sample_xml_paths.items():
            ...
            ingestor = factory.create_ingestor_for_file(str(xml_path), enable_validation=False)
            ...
            try:
                ...
                assert bundle_result['total'] >= 1, "Bundle should contain at least one resource"
                
                # Check that primary Claim resource exists
                resource_types = [entry['resource']['resourceType'] for entry in bundle_result['entry']]
                assert 'Claim' in resource_types, "Bundle should contain a Claim resource"
                
            except Exception as e:
                pytest.skip(f"FHIR Bundle validation skipped for {format_name}: {str(e)}")
    ...
    def test_diagnosis_codes_format(self):
        ...
        for diag in diagnoses:
            ...
            assert code[1:3].isdigit(), (
                f"ICD-10 must have digits at positions 2-3: {code}"
            )
...
class TestFHIRBundleSchema:
    ...
    def test_fhir_bundle_structure_validation(self):
        ...
        for format_name, xml_path in self.sample_files.items():
            ...
            factory_format = 'eClaimLink' if format_name == 'eclaim_link' else 'Shafafiya'
    ...
    def test_fhir_resource_extraction_completeness(self):
        ...
        for format_name, xml_path in self.sample_files.items():
            ...
            assert 'Claim' in resource_types, f"{format_name} should generate Claim resource"
            
            # Count unique resource types
            unique_types = set(resource_types)
            assert len(unique_types) >= 1, f"{format_name} should generate multiple resource types"
    ...
    def test_clinical_intelligence_extensions(self):
        ...
        for format_name, xml_path in self.sample_files.items():
            ...
            if 'extension' in bundle:
                ...
                assert len(nazmito_extensions) >= 0, "Should have clinical intelligence extensions"
                
                # Validate extension structure
                for ext in nazmito_extensions:
                    assert 'url' in ext
                    assert ext['url'].startswith('https://nazmito.com/fhir/StructureDefinition/')
            ...
            for entry in bundle['entry']:
                ...
                if resource_type in ['Observation', 'MedicationStatement', 'Procedure']:
                    ...
                    if 'subject' in resource:
                        ...
                        assert subject_ref.startswith('Patient/'), f"{resource_type} should reference Patient"
                    
                    # May also reference the claim via basedOn
                    if 'basedOn' in resource:
                        based_on_refs = resource['basedOn']
                        claim_refs = [ref for ref in based_on_refs if ref.get('reference', '').startswith('Claim/')]
    ...
    def test_clinical_data_quality_scoring(self):
        ...
        for format_name, xml_path in self.sample_files.items():
            ...
            if 'extension' in bundle:
                score_extensions = [
                    ext for ext in bundle['extension']
                    if any(score_type in ext.get('url', '') for score_type in [
                        'clinical-context-score', 'data-quality-score', 
                        'enrichment-score', 'ai-confidence'
                    ])
                ]
            ...
            if 'extension' in bundle:
                ...
                for uae_ext in uae_extensions:
                    if 'valueString' in uae_ext:
                        ...
                        assert authority in valid_authorities, f"Invalid UAE authority: {authority}"
                ...
                if resource['resourceType'] == 'Claim' and 'item' in resource:
                    for item in resource['item']:
                        if 'unitPrice' in item and 'currency' in item['unitPrice']:
                            ...
                            assert currency == 'AED', f"UAE should use AED currency, got {currency}"
