...
from api.services.claude_analysis_service import (
...
class TestClaudeAnalysisService:
        ...
        self.sample_bundle = {
            ...
            "fhir_resources": {
                "claim-1": {
                    ...
                    "item": [
                        {"productOrService": {"code": "83036", "display": "HbA1c test"}}
                    ],
                }
            },
            "raw_data": {
    ...
    def mock_claude_client(self):
        ...
        with patch("api.services.claude_analysis_service.anthropic") as mock_anthropic:
            ...
            # Use AsyncMock for async create
    ...
    async def test_analyze_patient_data_api_error(self, mock_claude_client):
        ...
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
            ...
            # Mock API error
            mock_claude_client.messages.create.side_effect = Exception(
    ...
    def test_estimate_analysis_cost_multiple_bundles(self):
        ...
        assert (
            estimated_cost < single_cost * 10
        )  # But not linearly more due to efficiency
    ...
    async def test_analyze_with_history_context(self, mock_claude_client):
        ...
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
            ...
            historical_bundle = {
                ...
                "raw_data": {"JustificationText": "Previous diabetes monitoring"},
            }
            ...
            assert (
                "historical" in prompt_content.lower()
                or "previous" in prompt_content.lower()
            )
