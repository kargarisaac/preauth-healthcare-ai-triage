"""
Test for Unified Data Pipeline with BAML LLM Functions (Mock Version)
===================================================================

This test validates that the new unified data pipeline correctly:
1. Creates unified patient records with Patient_007 data
2. Calls BAML functions successfully (with mock responses)
3. Validates the unified record structure has no data duplication
4. Confirms proper error handling when API calls fail

This version provides mock responses to avoid needing API keys.
"""
...
def create_mock_baml_client():
    """Create a mock BAML client that returns realistic responses."""
    mock_client = MagicMock()
    
    # Mock risk assessment response
    mock_risk_result = MagicMock()
    mock_risk_result.overall_risk = "MODERATE"
    mock_risk_result.risk_factors = ["Type 1 diabetes mellitus", "Young adult requiring insulin therapy", "Polypharmacy with three active medications"]
    mock_risk_result.confidence = 0.85
    mock_risk_result.reasoning = "Patient has Type 1 diabetes mellitus requiring multiple insulin preparations. At age 34, requires careful monitoring for diabetes complications. Current regimen includes rapid-acting insulin aspart, long-acting insulin glargine, and emergency glucagon kit, indicating comprehensive diabetes management but also higher clinical complexity."
    
    # Mock data quality response
    mock_quality_result = MagicMock()
    mock_quality_result.overall_score = 0.87
    mock_quality_result.completeness_score = 1.0
    mock_quality_result.richness_score = 0.85
    mock_quality_result.accuracy_score = 0.78
    mock_quality_result.recommendations = [
        "Consider adding recent HbA1c values for diabetes monitoring",
        "Include blood pressure readings for cardiovascular risk assessment",
        "Document diabetes complications screening results"
    ]
    mock_quality_result.reasoning = "Demographics are 100% complete with all required and optional fields populated. Clinical data shows good richness with 22 observations and 3 medications documented. Some accuracy concerns due to missing recent diabetes monitoring parameters that would be expected for Type 1 diabetes management."
    
    # Configure mock methods
    mock_client.AssessClinicalRisk.return_value = mock_risk_result
    mock_client.CalculateDataQuality.return_value = mock_quality_result
    
    return mock_client


def test_unified_data_pipeline_with_mock_baml():
    """
    Test unified data pipeline with mocked BAML responses.
    """
    print("=" * 60)
    print("Testing Unified Data Pipeline with Mock BAML Integration")
    print("=" * 60)
    ...
    # 2. Test unified patient record creation with mocked BAML
    print("\n2. Testing unified patient record creation with mock BAML...")
    
    # Create mock BAML client
    mock_client = create_mock_baml_client()
    
    try:
        # Patch the BAML client in the unified_data module
        with patch('data_ingestion.unified_data.b', mock_client):
        ...
        print("✅ Unified patient record created successfully with mocked BAML")
    ...
    # 4. Test BAML function execution (mocked)
    print("\n4. Testing BAML function execution with mock responses...")
    ...
    # Validate mock responses worked
    if risk_assessment.get('overall_risk') != "MODERATE":
        print(f"❌ Unexpected risk level: {risk_assessment.get('overall_risk')}")
        return False
    
    if risk_assessment.get('confidence') != 0.85:
        print(f"❌ Unexpected confidence: {risk_assessment.get('confidence')}")
        return False
        
    print("✅ Risk assessment mock working correctly")
    ...
    if data_quality.get('overall_score') != 0.87:
        print(f"❌ Unexpected quality score: {data_quality.get('overall_score')}")
        return False
        
    print("✅ Data quality assessment mock working correctly")
    ...
    # 7. Test that mock functions were called
    print("\n7. Validating BAML function calls...")
    
    # Check that mock functions were called with correct parameters
    mock_client.AssessClinicalRisk.assert_called_once()
    mock_client.CalculateDataQuality.assert_called_once()
    
    # Verify call arguments
    risk_call_args = mock_client.AssessClinicalRisk.call_args[1]
    quality_call_args = mock_client.CalculateDataQuality.call_args[1]
    
    if 'demographics' not in risk_call_args:
        print("❌ Demographics not passed to risk assessment")
        return False
        
    if 'demographics_completeness' not in quality_call_args:
        print("❌ Demographics completeness not passed to quality assessment")
        return False
    
    print("✅ BAML functions called with correct parameters")
    ...
    print("✅ BAML functions mocked and called successfully")
    print("✅ Mock responses validated correctly")
    ...
    print("   (Test used mock responses to avoid requiring API keys)")
    
    return True


if __name__ == "__main__":
    success = test_unified_data_pipeline_with_mock_baml()
